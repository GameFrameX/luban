...
class EditorJsonLoad : ITypeFuncVisitor<string, string, string>
{
    ...
    {
        return
            $"if({json}.IsString) {{ {x} = ({type.Apply(EditorDeclaringTypeNameVisitor.Ins)})System.Enum.Parse(typeof({type.Apply(EditorDeclaringTypeNameVisitor.Ins)}), {json}); }} else if({json}.IsNumber) {{ {x} = ({type.Apply(EditorDeclaringTypeNameVisitor.Ins)})(int){json}; }} else {{ throw new SerializationException(); }}  ";
    }
    ...
    public string Accept(TLang type, string json, string x)
    {
        return $"if(!{json}.IsString) {{ throw new SerializationException(); }}  {x} = {json};";
    }
    ...
    {
        return
            $"if(!{json}.IsArray) {{ throw new SerializationException(); }} int _n = {json}.Count; {x} = new {type.ElementType.Apply(EditorDeclaringTypeNameVisitor.Ins)}[_n]; int _index=0; foreach(SimpleJSON.JSONNode __e in {json}.Children) {{ {type.ElementType.Apply(EditorDeclaringTypeNameVisitor.Ins)} __v;  {type.ElementType.Apply(this, "__e", "__v")}  {x}[_index++] = __v; }}  ";
    }

    public string Accept(TList type, string json, string x)
    {
        return
            $"if(!{json}.IsArray) {{ throw new SerializationException(); }} {x} = new {type.Apply(EditorDeclaringTypeNameVisitor.Ins)}(); foreach(JSONNode __e in {json}.Children) {{ {type.ElementType.Apply(EditorDeclaringTypeNameVisitor.Ins)} __v;  {type.ElementType.Apply(this, "__e", "__v")}  {x}.Add(__v); }}  ";
    }
