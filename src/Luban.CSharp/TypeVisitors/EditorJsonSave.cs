...
class EditorJsonSave : ITypeFuncVisitor<string, string, string, string>
{
    ...
    public string Accept(TLang type, string jsonName, string jsonFieldName, string value)
    {
        return $"{jsonName}[\"{jsonFieldName}\"] = new JSONString({value});";
    }
    ...
    {
        return
            $"{{ var __bjson = new JSONObject();  {type.Apply(EditorUnderlyingTypeNameVisitor.Ins)}.SaveJson{type.DefBean.Name}({value}, __bjson); {jsonName}[\"{jsonFieldName}\"] = __bjson; }}";
    }

    public string Accept(TArray type, string jsonName, string jsonFieldName, string value)
    {
        return
            $"{{ var __cjson = new JSONArray(); foreach(var _e in {value}) {{ {type.ElementType.Apply(this, "__cjson", "null", "_e")} }} {jsonName}[\"{jsonFieldName}\"] = __cjson; }}";
    }
    ...
    {
        return
            $"{{ var __cjson = new JSONArray(); foreach(var _e in {value}) {{ var __entry = new JSONArray(); __cjson[null] = __entry; {type.KeyType.Apply(this, "__entry", "null", "_e.Key")} {type.ValueType.Apply(this, "__entry", "null", "_e.Value")} }} {jsonName}[\"{jsonFieldName}\"] = __cjson; }}";
    }
}
