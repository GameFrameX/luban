...
public abstract class LuaSplitCodeTargetBase : LuaCodeTargetBase
{
    ...
    public override void Handle(GenerationContext ctx, OutputFileManifest manifest)
    {
        ...
        var tasks = new List<Task<OutputFile>>
        {
            Task.Run(() =>
            {
                ...
                return CreateOutputFile($"{ctx.Target.Manager}.def.lua", writer.ToResult(FileHeader));
            })
        };
        ...
        foreach (var table in ctx.ExportTables)
        {
            tasks.Add(Task.Run(() =>
            {
                ...
                return CreateOutputFile($"{table.FullName}.def.lua", writer.ToResult(FileHeader));
            }));
        }
        ...
        foreach (var bean in ctx.ExportBeans)
        {
            tasks.Add(Task.Run(() =>
            {
                ...
                return CreateOutputFile($"{bean.FullName}.def.lua", writer.ToResult(FileHeader));
            }));
        }
