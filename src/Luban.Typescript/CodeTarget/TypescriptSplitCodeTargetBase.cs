...
public abstract class TypescriptSplitCodeTargetBase : TypescriptCodeTargetBase
{
    ...
    public override void Handle(GenerationContext ctx, OutputFileManifest manifest)
    {
        ...
        tasks.Add(Task.Run(() =>
        {
            ...
            return CreateOutputFile($"{ctx.Target.Manager}.def.ts", writer.ToResult(FileHeader));
        }));
        ...
        foreach (var table in ctx.ExportTables)
        {
            tasks.Add(Task.Run(() =>
            {
                ...
                return CreateOutputFile($"{table.FullName}.def.ts", writer.ToResult(FileHeader));
            }));
        }
        ...
        foreach (var bean in ctx.ExportBeans)
        {
            tasks.Add(Task.Run(() =>
            {
                ...
                return CreateOutputFile($"{bean.FullName}.def.ts", writer.ToResult(FileHeader));
            }));
        }
