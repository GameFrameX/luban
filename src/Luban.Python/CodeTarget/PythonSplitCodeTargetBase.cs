...
public abstract class PythonSplitCodeTargetBase : PythonCodeTargetBase
{
    ...
    public override void Handle(GenerationContext ctx, OutputFileManifest manifest)
    {
        ...
        var tasks = new List<Task<OutputFile>>
        {
            Task.Run(() =>
            {
                ...
                return CreateOutputFile($"{ctx.Target.Manager}.def.py", writer.ToResult(FileHeader));
            })
        };
        ...
        foreach (var table in ctx.ExportTables)
        {
            tasks.Add(Task.Run(() =>
            {
                ...
                return CreateOutputFile($"{table.FullName}.def.py", writer.ToResult(FileHeader));
            }));
        }
        ...
        foreach (var bean in ctx.ExportBeans)
        {
            tasks.Add(Task.Run(() =>
            {
                ...
                return CreateOutputFile($"{bean.FullName}.def.py", writer.ToResult(FileHeader));
            }));
        }
